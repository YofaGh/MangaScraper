...
class SetChapters(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        if self.dest == "r" and values[0] > values[1]:
            raise ValueError(
                "The beginning chapter must be lower than the ending chapter"
            )
        if self.dest == "l":
            values = [values]
...
parser.add_argument(
    "task",
    choices=("manga", "doujin", "merge", "c2pdf", "search", "db", "check", "sauce"),
)
webtoon_type = parser.add_argument_group("download").add_mutually_exclusive_group()
webtoon_type.add_argument(
    "-single", "-code", help="url of the manga, or code of the doujin"
)
webtoon_type.add_argument(
    "-file", help="downloads webtoons based on the given json file"
)
mc_options = parser.add_argument_group(
    "merge or convert"
).add_mutually_exclusive_group()
mc_options.add_argument("-folder", help="merges or converts images in given folder")
mc_options.add_argument(
    "-bulk", help="merges or converts images of folders in the given folder"
)
mc_options.add_argument(
    "-bulkone", help="converts images of folders in the given folder into one pdf file"
)
parser.add_argument(
    "-s", nargs="+", metavar="modules", help="specify domains to scrape from"
)
...
parser.add_argument(
    "-fit", action="store_true", help="if set, resizes the images to the same width"
)
parser.add_argument(
    "-p", action="store_true", help="if set, converts images to a pdf file"
)
...
chapters.add_argument(
    "-c", action=SetChapters, nargs="+", type=float, help="specify chapters"
)
chapters.add_argument(
    "-l", action=SetChapters, type=float, help="chapters after the given chapter"
)
chapters.add_argument(
    "-r",
    action=SetChapters,
    ...
    type=float,
    metavar=("begin", "end"),
    help="chapters between the given chapters",
)
search_args = parser.add_argument_group("customize search results")
search_args.add_argument(
    "-page-limit", default=3, type=int, help="specify how many pages should be searched"
)
search_args.add_argument(
    "-absolute",
    action="store_true",
    help="if set, checks that the name you searched should be in the title",
)
saucer = parser.add_argument_group(
    "find source of an image"
).add_mutually_exclusive_group()
...
match args.task:
    case "manga":
        if args.file:
            ...
            download_single(
                args.n or args.single, args.single, args.s[0], args.l, args.r, args.c
            )
        else:
    ...
    case "c2pdf":
        if args.folder:
            ...
            parser.error(
                "please set one of the following arguments: [-folder, -bulk, -bulkone]"
            )
