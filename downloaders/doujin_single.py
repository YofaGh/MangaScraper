...
def download_doujin(code: str, module: Doujin) -> bool:
    ...
    try:
        ...
        doujin_title = (
            f"{code}_{module.get_title(code)}"
            if module.is_coded
            else module.get_title(code)
        )
        ...
        while i < len(images):
            log_over(
                f"\r{shorten_doujin_title}: Downloading image {i + 1}/{len(images)}..."
            )
            if save_names:
                save_path = f"{fixed_doujin_name}/{save_names[i]}"
            else:
                save_path = (
                    f"{fixed_doujin_name}/{i + 1:03d}.{images[i].split('.')[-1]}"
                )
            if not os.path.exists(save_path):
                ...
                if not saved_path:
                    log(
                        f" Warning: Image {i + 1} could not be downloaded. url: {images[i]}",
                        "red",
                    )
                elif not validate_corrupted_image(saved_path):
                    log(
                        f" Warning: Image {i + 1} was corrupted. may not be able to merge this chapter.",
                        "red",
                    )
                elif (
                    ...
                    log(
                        f" Warning: Image {i + 1} was truncated. trying to download it one more time...",
                        "red",
                    )
                    continue
            i += 1
        log(
            f"\r{shorten_doujin_title}: Finished downloading, {len(images)} images were downloaded.",
            "green",
        )
        if AUTO_MERGE:
            ...
            merge_folder(
                fixed_doujin_name, f"Merged/{fixed_doujin_name}", shorten_doujin_title
            )
