...
class Manga18(Manga):
    ...
    def get_info(self, manga):
        ...
        with suppress(Exception):
            summary = soup.find("div", {"class": "detail_reviewContent"}).get_text(
                strip=True
            )
        with suppress(Exception):
            rating = float(
                info_box.find("div", {"class": "detail_rate"})
                .find("span")
                .get_text(strip=True)
                .replace("/5", "")
            )
        with suppress(Exception):
            alternative = (
                item_box.find(lambda tag: "Other name" in tag.text)
                .find("span")
                .get_text(strip=True)
            )
        with suppress(Exception):
            status = (
                item_box.find(lambda tag: "Status" in tag.text)
                .find("span")
                .get_text(strip=True)
            )
        with suppress(Exception):
            authors = [
                ...
                for a in item_box.find(lambda tag: "Author" in tag.text).find_all("a")
            ]
        with suppress(Exception):
            artists = [
                a.get_text(strip=True)
                for a in item_box.find(lambda tag: "Artist" in tag.text).find_all("a")
            ]
        with suppress(Exception):
            views = (
                item_box.find(lambda tag: "Views" in tag.text)
                .find("span")
                .get_text(strip=True)
            )
        with suppress(Exception):
            categories = [
                a.get_text(strip=True)
                for a in item_box.find(lambda tag: "Categories" in tag.text).find_all(
                    "a"
                )
            ]
        return {
    ...
    def search_by_keyword(self, keyword, absolute):
        ...
        while True:
            ...
            for manga in mangas:
                ...
                with suppress(Exception):
                    latest_chapter = (
                        manga.find("div", {"class": "mg_chapter"})
                        .find("a")["href"]
                        .split("/")[-1]
                    )
