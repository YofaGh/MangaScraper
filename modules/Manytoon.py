...
class Manytoon(Manga):
    ...
    def get_info(manga):
        ...
        with suppress(Exception):
            rating = float(info_box.find('span', {'class': 'score font-meta total_votes'}).get_text(strip=True))
        with suppress(Exception):
    ...
    def get_chapters(manga):
        ...
        response, _ = Manytoon.send_request('https://manytoon.com/wp-admin/admin-ajax.php', method='POST', session=session, data=data)
        soup = BeautifulSoup(response.text, 'html.parser')
    ...
    def search_by_keyword(keyword, absolute):
        ...
        while True:
            ...
            for manga in mangas:
                ...
                latest_chapter = ''
                with suppress(Exception):
                    latest_chapter = manga.find('span', {'class': 'chapter font-meta'}).find('a')['href'].split('/')[-2]
                results[ti] = {
