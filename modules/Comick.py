...
class Comick(Manga):
    ...
    def get_info(manga):
        ...
        extras = {
            item["md_genres"]["group"]: [
                d["md_genres"]["name"]
                for d in data["comic"]["md_comic_md_genres"]
                if d.get("md_genres", {}).get("group") == item["md_genres"]["group"]
            ]
            for item in data["comic"]["md_comic_md_genres"]
        }
        ...
        extras["Publishers"] = [
            ti["mu_publishers"]["title"]
            for ti in data["comic"]["mu_comics"]["mu_comic_publishers"]
        ]
        extras["Categories"] = [
            ti["mu_categories"]["title"]
            for ti in data["comic"]["mu_comics"]["mu_comic_categories"]
        ]
        return {
            "Cover": f"https://meo3.comick.pictures/{data['comic']['md_covers'][0]['b2key']}",
            "Title": data["comic"]["title"],
            "Alternative": ", ".join(
                [ti["title"] for ti in data["comic"]["md_titles"]]
            ),
    ...
    def get_chapters(manga):
        ...
        while True:
            ...
            if not response["chapters"]:
                break
            chapters_urls.extend(
                [
                    f"{chapter['hid']}-chapter-{chapter['chap']}-en"
    ...
    def search_by_keyword(keyword, absolute):
        ...
        genres = {
            genre["id"]: genre["name"]
            for genre in json.loads(script)["props"]["pageProps"]["genres"]
        }
        page = 1
        while True:
            try:
            ...
            for manga in mangas:
                ...
                results[manga["title"]] = {
                    ...
                    "genres": ", ".join(
                        [genres[genre_id] for genre_id in manga["genres"]]
                    ),
                    "page": page,
                }
            yield results
