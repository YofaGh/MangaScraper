...
class Vyvymanga(Manga):
    ...
    def get_info(manga):
        ...
        with suppress(Exception):
            alternative = box.find('div', {'class': 'col-md-7'}).find('p').get_text(strip=True)
        with suppress(Exception):
            summary = box.find('div', {'class': 'summary'}).find('p', {'class': 'content'}).get_text(strip=True)
        with suppress(Exception):
            authors = [a.get_text(strip=True) for a in bbox.find(lambda tag: tag.name == 'p' and 'Authors' in tag.text).find_all('a')]
        with suppress(Exception):
            status = bbox.find(lambda tag: tag.name == 'p' and 'Status' in tag.text).contents[-1].get_text(strip=True)
        with suppress(Exception):
            view = bbox.find(lambda tag: tag.name == 'p' and 'View' in tag.text).contents[-1].get_text(strip=True)
        with suppress(Exception):
            genres = [a.get_text(strip=True) for a in bbox.find(lambda tag: tag.name == 'p' and 'Genres' in tag.text).find_all('a')]
        with suppress(Exception):
            rating = float(bbox.find(lambda tag: tag.name == 'p' and 'Rating' in tag.text).contents[-1].split('/')[0].strip())/2
        return {
            'Cover': cover,
            'Title': title,
    ...
    def search_by_keyword(keyword, absolute):
        ...
        while True:
            ...
            for manga in mangas:
                ...
                with suppress(Exception):
                    latest_chapter = manga.find('span', {'class': 'tray-item'}).get_text(strip=True)
                with suppress(Exception):
