...
class Truemanga(Manga):
    ...
    def get_info(self, manga):
        ...
        with suppress(Exception):
            summary = (
                soup.find("div", {"class": "section-body summary"})
                .find("p")
                .get_text(strip=True)
            )
        for p in info_box.find("div", {"class": "meta box mt-1 p-10"}).find_all("p"):
            if "Status" in p.get_text():
                ...
                if len(p.find_all("a")) <= 1:
                    ...
                    ] = p.find("span").get_text(strip=True)
                else:
                    extras[
                        p.find("strong").get_text(strip=True).replace(":", "").strip()
                    ] = [
    ...
    def search_by_keyword(self, keyword, absolute):
        ...
        template = (
            f"https://truemanga.com/search?q={keyword}&page=P_P_P_P"
            if keyword
            else "https://truemanga.com/az-list?page=P_P_P_P"
        )
        page = 1
        session = None
        while True:
