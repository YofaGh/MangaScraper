...
class Daycomics(Manga):
    ...
    def get_info(manga):
        ...
        with suppress(Exception):
            summary = (
                info_box.find("div", {"class": "title_content"})
                .find_all("h2")[-1]
                .get_text(strip=True)
            )
        with suppress(Exception):
            extras["Genres"] = [
                genre.strip()
                for genre in info_box.find("p", {"class": "type_box"})
                .find("span", {"class": "type"})
                .get_text(strip=True)
                .split(" / ")
            ]
        with suppress(Exception):
            extras["Authors"] = (
                info_box.find("p", {"class": "type_box"})
                .find("span", {"class": "writer"})
                .get_text(strip=True)
            )
        return {
            "Cover": cover,
    ...
    def search_by_keyword(keyword, absolute):
        ...
        session = None
        template = (
            f"https://daycomics.me/en/search?keyword={keyword}&"
            if keyword
            ...
            for manga in mangas:
                ...
                with suppress(Exception):
                    status = (
                        "Finished"
                        if "End"
                        in manga.find("p", {"class": "ico-box"}).get_text(strip=True)
                        else "Ongoing"
                    )
                with suppress(Exception):
