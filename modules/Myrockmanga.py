...
class Myrockmanga(Manga):
    ...
    def get_info(manga):
        ...
        with suppress(Exception):
            title = soup.find("h1", {"class": "title text-lg-left"}).get_text(
                strip=True
            )
        with suppress(Exception):
            alternative = (
        ...
        with suppress(Exception):
            summary = (
                soup.find("div", {"class": "summary"}).find("p").get_text(strip=True)
            )
        with suppress(Exception):
            rating = (
                float(
                    soup.find("div", {"class": "rating"})
                    .find("span")
                    .get_text(strip=True)
                    .replace("/ 10", "")
                )
        ...
        with suppress(Exception):
            genres = [
                ...
                for a in soup.find("div", {"class": "genres"}).find_all("a")
            ]
        for box in soup.find("table").find_all(
            lambda tag: tag.name == "tr" and tag.find("th") and tag.find("td")
        ):
            if "Status" in box.text:
                status = box.find("td").get_text(strip=True)
            else:
                extras[box.find("th").get_text(strip=True)] = [
    ...
    def search_by_keyword(keyword, absolute):
        ...
        while True:
            ...
            for section in sections:
                if (
                    section.find("div", {"class": "collection shadow-z-1-home"})
                    and section.find("h4", {"class": "group-header"}).get_text(
                        strip=True
                    )
                    == "Manga"
                ):
                    mangas = section.find_all(
            ...
            for manga in mangas:
                ...
                with suppress(Exception):
                    lang = (
                        header.find("img", {"class": "flag"})["src"]
                        .split("/")[-1]
                        .split(".")[0]
                    )
                with suppress(Exception):
