        ...
        while True:
            ...
            for doujin in doujins:
                ...
                if absolute and keyword.lower() not in ti.lower():
                    continue
                code = doujin.find('td', {'class': 'gl3c glname'}).find('a')['href'].replace('https://e-hentai.org/g/', '')[:-1]
                thumbnail = doujin.find('img')
                thumbnail = thumbnail['data-src'] if thumbnail.has_attr('data-src') else thumbnail['src']
                results[ti] = {
                    'domain': E_Hentai.domain,
                    'code': code,
                    'category': doujin.find('div', {'class':'cn'}).get_text(strip=True),
                    'thumbnail': thumbnail,
                    'page': page
                }
                last = code.split('/')[0]
