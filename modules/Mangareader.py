...
class Mangareader(Manga):
    ...
    def get_info(manga):
        ...
        with suppress(Exception):
            status = info_box.find_all(lambda tag: tag.name == 'li' and 'Status' in tag.text)[2].get_text(strip=True).replace('Status:', '').strip()
        with suppress(Exception):
            authors = info_box.find_all(lambda tag: tag.name == 'li' and 'Author' in tag.text)[2].get_text(strip=True).replace('Author:', '')
        with suppress(Exception):
            views = info_box.find_all(lambda tag: tag.name == 'li' and 'Views' in tag.text)[2].get_text(strip=True).replace('Views:', '').replace('Views:', '')
        with suppress(Exception):
            genres = [a.get_text(strip=True) for a in info_box.find_all(lambda tag: tag.name == 'li' and 'Genres' in tag.text)[2].find_all('a')]
        return {
            'Cover': cover,
            'Title': title,
