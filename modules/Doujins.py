    ...
    def get_info(self, code):
        ...
        with suppress(Exception):
            artists = [
                ...
                for a in soup.find("div", {"class": "gallery-artist"}).find_all("a")
            ]
        with suppress(Exception):
            tags = [
                a.get_text(strip=True)
                for a in soup.find("li", {"class": "tag-area"}).find_all("a")
            ]
        with suppress(Exception):
            pages = (
                soup.find_all("div", {"class": "folder-message"})[1]
                .get_text(strip=True)
                .split("•")[-1]
                .replace("images", "")
                .strip()
        ...
        with suppress(Exception):
            translated_by = (
                soup.find("div", {"class": "folder-message"})
                .get_text(strip=True)
        ...
        return {
            ...
            "Dates": {
                "Uploaded": datetime.strptime(uploaded, "%B %dth, %Y").strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            },
        }
