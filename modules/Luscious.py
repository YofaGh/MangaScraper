...
class Luscious(Doujin):
    ...
    def get_images(code):
        ...
        response, session = Luscious.send_request(data.replace('__album__id__', str(code)).replace('__page__number__', '1')).json()
        ...
        for page in range(2,total_pages + 1):
            response, session = Luscious.send_request(data.replace('__album__id__', str(code)).replace('__page__number__', str(page)), session=session).json()
