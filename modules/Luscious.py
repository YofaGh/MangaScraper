...
class Luscious(Doujin):
    ...
    def get_images(code):
        ...
        response = Luscious.send_request(data.replace('__album__id__', str(code)).replace('__page__number__', '1'))
        response_raw = json.loads(response.text)
        total_pages = response_raw['data']['picture']['list']['info']['total_pages']
        images = [item['url_to_original'] for item in response_raw['data']['picture']['list']['items']]
        for page in range(2,total_pages + 1):
            response = Luscious.send_request(data.replace('__album__id__', code).replace('__page__number__', str(page)))
            response_raw = json.loads(response.text)
            new_images = [item['url_to_original'] for item in response_raw['data']['picture']['list']['items']]
    ...
    def search_by_keyword(keyword, absolute):
        ...
        while True:
            if page > total_pages:
                yield {}
            try:
                response = Luscious.send_request(data.replace('__keyword__', keyword).replace('__page__number__', str(page)))
                response_raw = json.loads(response.text)
                total_pages = response_raw['data']['album']['list']['info']['total_pages']
            except HTTPError:
                yield {}
            doujins = response_raw['data']['album']['list']['items']
            results = {}
