...
class Hentairead(Doujin):
    ...
    def get_info(code):
        ...
        with suppress(Exception):
            cover = (
                soup.find("div", {"class": "c-image-hover"})
                .find("img")["data-src"]
                .rsplit("?", 1)[0]
            )
        with suppress(Exception):
        ...
        with suppress(Exception):
            rating = float(
                soup.find("span", {"class": "score font-meta total_votes"})
                .get_text(strip=True)
                .split(" ")[0]
            )
        tag_box = soup.find("div", {"class": "post-summary"}).find_all(
            "div", {"class": "post-meta"}
        )
        for box in tag_box:
            if "Pages" in box.text:
                ...
                with suppress(Exception):
                    uploaded = box.find("span", {"class": "post-meta-value"}).get_text(
                        strip=True
                    )
            elif "Views" in box.text:
                with suppress(Exception):
                    extras["Views"] = box.find("div", {"class": "tag-name"}).get_text(
                        strip=True
                    )
            else:
                with suppress(Exception):
                    extras[box.find("span").contents[0].strip()] = [
                        tag.get_text(strip=True)
                        for tag in box.find_all("span", {"class": "tag-name"})
                    ]
        return {
            "Cover": cover,
            "Title": title,
            ...
            "Extras": extras,
            "Dates": {
                "Uploaded": datetime.strptime(uploaded, "%d/%m/%Y %H:%M").strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            },
        }
    ...
    def search_by_keyword(keyword, absolute):
        ...
        while True:
            ...
            for doujin in doujins:
                ...
                with suppress(Exception):
                    alternative = doujin.find(
                        "div", {"class": "item-alter-title"}
                    ).get_text(strip=True)
                with suppress(Exception):
                    parody = doujin.find("div", {"class": "item-parody"}).get_text(
                        strip=True
                    )
                with suppress(Exception):
                    artist = doujin.find("div", {"class": "item-by-artist"}).get_text(
                        strip=True
                    )
                with suppress(Exception):
                    tags = [
                        tag.get_text(strip=True)
                        for tag in doujin.find_all("span", {"class": "tag-name"})
                    ]
                with suppress(Exception):
                    views = doujin.find("div", {"class": "item__total-views"}).get_text(
                        strip=True
                    )
                with suppress(Exception):
                    rating = float(
                        doujin.find("div", {"class": "item__rating"}).get_text(
                            strip=True
                        )
                    )
                with suppress(Exception):
                    total_pages = int(
                        doujin.find("div", {"class": "item__total-pages"}).get_text(
                            strip=True
                        )
                    )
