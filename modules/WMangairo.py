...
class WMangairo(Manga):
    ...
    def get_info(manga):
        ...
        with suppress(Exception):
            summary = (
                soup.find("div", {"id": "story_discription"})
                .find("p")
                .contents[5]
                .replace("\n", "")
                .replace("\xa0", "")
        ...
        with suppress(Exception):
            last_updated = datetime.strptime(
                contents[5].contents[-1].get_text(strip=True), "%b-%d-%Y %H:%M"
        ...
        return {
            ...
            "Summary": summary,
            "Status": status,
            "Extras": {"Authors": authors, "Genres": genres, "View": view},
    ...
    def search_by_keyword(keyword, absolute):
        ...
        while True:
            ...
            for manga in mangas:
                ...
                with suppress(Exception):
                    latest_chapter = manga.find("a", {"class": "chapter-name"})[
                        "href"
                    ].split("/")[-1]
                with suppress(Exception):
                    authors = manga.find(
