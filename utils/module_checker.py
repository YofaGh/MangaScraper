...
def check_modules(domains: list[str] | None = None) -> None:
    ...
    for domain in domains:
        ...
        se_db_checker(
            module,
            "search_by_keyword",
            modules[domain]["test_sample"].get("keyword", "a"),
            False,
        )
...
def check_manga(module: Manga, sample: dict) -> None:
    ...
    if chapters:
        images, save_names = get_images_checker(
            module,
            f"{sample['url']}: {chapters[0]['name']}",
            sample["url"],
            chapters[0],
        )
    else:
        log(
            f"\r{module.domain}: {sample['url']}: Skipped images_checker due to chapter_checker failure",
            "red",
        )
    if images:
        download_checker(
            ...
            save_names[0]
            if save_names
            else f"{module.domain}_test.{images[0].split('.')[-1]}",
        )
    else:
        log(
            f"\r{module.domain}: {sample['url']}: Skipped download_checker due to images_checker failure",
            "red",
        )
...
def get_images_checker(
    module: Manga | Doujin, info: str, *args
...
def download_checker(module: Manga | Doujin, url: str, name: str) -> None:
    ...
    try:
        ...
        if validate_corrupted_image(saved_path) and validate_truncated_image(
            saved_path
        ):
